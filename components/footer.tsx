...
const Footer = () => {
  ...
  const [pageContent, setPageContent] = useState<pageContentProps | null>(null);
  ...
  const getContent = async () => {
    try {
      const res = await axios.get(`${process.env.API_URL}/get-site-settings`);
      if (res.status === 200) {
      ...
      <footer id={style.footer}>
        <div className={style.contain}>
          <div className={style.main_row + " row justify-content-between"}>
            ...
            <div className={style.column4}>
              <div className={style.title}>
                Sign up to our news letter to receive updates on our latest
                tournament events
              </div>
              <form action="" method="POST">
                ...
                <p>
                  ...
                  <Link href="/privacy-policy">Google</Link>,{" "}
                  <Link href="/privacy-policy">Privacy Policy</Link> and{" "}
                  <Link href="/privacy-policy">Terms of Service</Link> apply.
                </p>
              </form>
            </div>
          </div>
          <div className={style.copyright}>
            <p>
              © {currentDate.getFullYear()} <Link href="/">Eventplus</Link>. All
              Rights Reserved
            </p>
