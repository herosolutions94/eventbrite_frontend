...
const ProfileForm = ({ profileData }: ProfileFormProps) => {
  ...
  const handleSubmitForm = async (e: any) => {
    ...
    const newFrmData = {
      ...formData,
      user_image: userThumbnail,
      type: "organizer",
      user_cover: userCover,
    };
    try {
      const response = await axios.post(
        process.env.API_URL + "/update-user-profile",
        newFrmData
      );
  ...
  const handleUploadCover = async (e: any) => {
    ...
    try {
      const response = await axios.post(
        process.env.API_URL + "/upload-cover",
        fd
      );
      toast.success(response.data.message);
      if (response) {
        ...
        <div className={style.blk}>
          ...
          >
            ...
            {coverLoading ? (
              ...
              <>
                <div
                  className={style.organizer_cover_choose}
            ...
            <div className="col-lg-6 col-6">
              ...
              <div className={style.form_blk}>
                <input
                  ...
                  autoComplete="none"
                  onChange={(e) =>
                    setFormData({ ...formData, org_website: e.target.value })
                  }
                />
            ...
            <div className="col-lg-4 col-6">
              ...
              <div className={style.form_blk}>
                <input
                  ...
                  autoComplete="none"
                  placeholder="eg: sample@gmail.com"
                  value={formData.org_mailing_address}
                  onChange={(e) =>
                    setFormData({
                      ...formData,
                      org_mailing_address: e.target.value,
                    })
                  }
                />
            ...
            <div className="col-lg-4 col-6">
              ...
              <div className={style.form_blk}>
                ...
                >
                  ...
                  <option
                    value="email"
                    selected={profileData?.org_communication_method === "email"}
                  >
                    Email
                  </option>
                  <option
                    value="phone"
                    selected={profileData?.org_communication_method === "phone"}
                  >
                    Phone
                  </option>
                  <option
                    value="messaging app"
                    selected={
                      profileData?.org_communication_method === "messaging app"
                    }
                  >
                    Messaging App
                  </option>
            ...
            <div className="col-lg-4 col-6">
              ...
              <div className={style.form_blk}>
                <input
                  ...
                  onChange={(e) =>
                    setFormData({ ...formData, org_timezone: e.target.value })
                  }
                />
                ...
                >
                  ...
                  {countriesData.map((country, index) =>
                    parseInt(formData?.country) === country.id ? (
                      <option value={country.id} key={index} selected={true}>
                        {country.name}
                      </option>
                    ) : (
                ...
                >
                  {statesData.map((state, index) =>
                    parseInt(formData?.state) === state.id ? (
                      <option value={state.id} key={index} selected={true}>
                        {state.name}
                      </option>
                    ) : (
          ...
          <div className={`${style.btn_blk} justify-content-center mt-5`}>
            <button
              type="submit"
              className={`${style.site_btn} ${style.px}`}
              onClick={handleSubmitForm}
              disabled={imageLoading || profileLoading}
            >
              Save {profileLoading ? <div className={style.spinner}></div> : ""}
            </button>
          </div>
        </div>
      </form>
